...
function Bleno() {
  ...
  this._bindings.on('phyUpdate', this.onPhySet.bind(this));
...
Bleno.prototype.setPhy = function(callback) {
  if (callback) {
    this.once('setPhy', function(phy) {
      callback(null, phy);
    });
  }

  this._bindings.setPhy();
};

Bleno.prototype.onPhySet = function(rssi) {
  this.emit('setPhy', rssi);
};
