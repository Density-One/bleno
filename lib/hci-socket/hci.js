...
var LE_SET_PHY_CMD = OCF_LE_SET_PHY | (OGF_LE_CTL << 10);
...
const LE_SET_EXTENDED_ADVERTISING_PARAMETERS_CMD = 0x2036;

const OCF_LE_SET_EXTENDED_ADVERTISE_ENABLE = 0x0039;

const LE_SET_EXTENDED_ADVERTISE_ENABLE_CMD = OCF_LE_SET_EXTENDED_ADVERTISE_ENABLE + (OGF_LE_CTL << 10);
...
Hci.prototype.setPhy = function(handle) {
  var cmd = Buffer.alloc(11); // Allocate buffer for command

  // Header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0); // Write packet indicator at position 0
  cmd.writeUInt16LE(LE_SET_PHY_CMD, 1); // Write the command opcode at position 1

  // Length
  cmd.writeUInt8(0x05, 3); // Write the length of the LE Set PHY command parameters

  // Command parameters
  cmd.writeUInt16LE(handle, 4); // Write the connection handle at position 4-5
  cmd.writeUInt8(0x00, 6); // Write ALL_PHYS parameter at position 6
  cmd.writeUInt8(0x02, 7); // Write TX_PHY parameter at position 7
  cmd.writeUInt8(0x02, 8); // Write RX_PHY parameter at position 8
  cmd.writeUInt16LE(0x0000, 9); // Write PHY_options at position 9-10

  // Logging and sending
  console.log('setLePhy - writing: ' + cmd.toString('hex'));
  debug('setLePhy - writing: ' + cmd.toString('hex'));
...
Hci.prototype.setAdvertisingParameters = function() {
  // The buffer size would be the sum of the command header (4 bytes) and the parameters length (21 bytes)
  var cmd = Buffer.alloc(4 + 25); // 4 bytes for header + 25 bytes for parameters

// Header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_SET_EXTENDED_ADVERTISING_PARAMETERS_CMD, 1);

// Length of the parameters
  cmd.writeUInt8(25, 3);

// Advertising handle (typically 0 for a single advertiser)
  cmd.writeUInt8(0x01, 4); // Handle set to 0x01 as per your example

// Advertising properties (0x0013 - Connectable, Scannable, Use legacy advertising PDUs)
  console.log('set extended advertising parameters1 - writing: ' + cmd.toString('hex'));
  cmd.writeUInt16LE(0x0013, 5);

// Primary advertising interval min (2048 units, which is 1280 ms)
  console.log('set extended advertising parameters2 - writing: ' + cmd.toString('hex'));
  cmd.writeUInt16LE(0x0800, 7);

// Primary advertising interval max (2048 units, which is 1280 ms)
  console.log('set extended advertising parameters3 - writing: ' + cmd.toString('hex'));
  cmd.writeUInt16LE(0x0000, 9);

// Primary advertising channel map (use all channels)
  console.log('set extended advertising parameters4 - writing: ' + cmd.toString('hex'));
  cmd.writeUInt16LE(0x0008, 11);
// Primary advertising channel map (use all channels)
  cmd.writeUInt8(0x07, 13); // Channel map set to 0x07 to use channels 37, 38, 39
  console.log('set extended advertising parameters5 - writing: ' + cmd.toString('hex'));

// Own address type (public)
  cmd.writeUInt8(0x00, 14);

// Peer address type (public)
  cmd.writeUInt8(0x00, 15);

// Peer address (set to zeros)
  Buffer.from('000000000000', 'hex').copy(cmd, 16);

// Advertising filter policy (Allow Scan Request from Any, Allow Connect Request from Any)
  cmd.writeUInt8(0x00, 22);

// Advertising TX power (Host has no preference)
  cmd.writeInt8(0x7F, 23);

// Primary advertising PHY (LE 1M)
  cmd.writeUInt8(0x01, 24);

// Secondary advertising max skip
  cmd.writeUInt8(0x00, 25);

// Secondary advertising PHY (LE 1M)
  cmd.writeUInt8(0x01, 26); // Changed to 0x01 as per your example

// Advertising SID (use 0 if unused)
  cmd.writeUInt8(0x00, 27);

// Scan Request Notification Enable (disabled = 0x00)
  cmd.writeUInt8(0x00, 28);

  console.log('set extended advertising parameters - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
...
Hci.prototype.setAdvertiseEnable = function(enabled) {
  // Determine the length of the command, including all advertising sets
  var handle = 16;
  var cmdLength = 7;
  var cmd = Buffer.alloc(cmdLength);

  // Header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_SET_EXTENDED_ADVERTISE_ENABLE_CMD, 1); // Correct opcode for "LE Set Extended Advertising Enable"

  // Length of the parameters
  cmd.writeUInt8(cmdLength - 4, 3); // Length of the command minus the header

  // Command parameters
  cmd.writeUInt8(enabled ? 0x01 : 0x00, 4); // Enable or disable advertising sets
  cmd.writeUInt8(handle.length, 5); // Number of sets

  // Set parameters for each advertising handle
  for (let i = 0; i < handle.length; i++) {
    cmd.writeUInt8(handle[i], 6 + (i * 2)); // Advertising handle
    cmd.writeUInt8(0x00, 7 + (i * 2)); // Duration and max extended advertising events (set to 0 for no limit)
  }

  debug('set extended advertise enable - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
