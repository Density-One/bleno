...
const OCF_LE_CONNECTION_UPDATE = 0x0013; // Opcode Command Field for LE Connection Update Command

const LE_CONNECTION_UPDATE_CMD = OCF_LE_CONNECTION_UPDATE | (OGF_LE_CTL << 10);
...
Hci.prototype.reset = function() {
  ...
  console.log('reset - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.sendDleUpdate= function() {
  var hciCommand = Buffer.alloc(8);
  hciCommand.writeUInt8(0x01, 0); // HCI Command packet type
  hciCommand.writeUInt16LE(0x2024, 1); // Opcode for LE Write Suggested Default Data Length (OGF = 0x08, OCF = 0x0024)
  hciCommand.writeUInt8(0x04, 3); // Length of the command parameters (4 bytes)

  // Max Tx Octets and Time
  var maxTxOctets = 251; // Maximum Tx Octets, e.g., 251
  var maxTxTime = 2120; // Maximum Tx Time in microseconds, e.g., 2120
  hciCommand.writeUInt16LE(maxTxOctets, 4); // Max Tx Octets
  hciCommand.writeUInt16LE(maxTxTime, 6); // Max Tx Time


  this._socket.write(hciCommand);

}


Hci.prototype.readMaxDataLength= function() {
  var HCI_COMMAND_PKT = 0x01;
  var OCF_LE_READ_MAXIMUM_DATA_LENGTH = 0x002F;
  var OGF_LE_CTL = 0x08;

  var LE_READ_MAXIMUM_DATA_LENGTH_CMD = (OGF_LE_CTL << 10) | OCF_LE_READ_MAXIMUM_DATA_LENGTH;

  var hciCommand = Buffer.alloc(4); // Buffer for command packet
  hciCommand.writeUInt8(HCI_COMMAND_PKT, 0); // HCI Command packet type
  hciCommand.writeUInt16LE(LE_READ_MAXIMUM_DATA_LENGTH_CMD, 1); // Opcode
  hciCommand.writeUInt8(0x00, 3); // Parameter length

// Output the command as a hexadecimal string for debugging
  console.log('LE Read Maximum Data Length command:', hciCommand.toString('hex'));

  this._socket.write(hciCommand);

}

Hci.prototype.sendConnectionUpdate = function(connectionHandle, minInterval, maxInterval) {
  var cmd = Buffer.alloc(18); // Allocate buffer for the command

  // Header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_CONNECTION_UPDATE_CMD, 1); // Opcode for LE Connection Update Command

  // Length
  cmd.writeUInt8(14, 3); // Length of the parameters (10 bytes)

  // TODO get Connection Handle (16 usually)
  cmd.writeUInt16LE(connectionHandle, 4);

  // Connection Interval Min (units of 1.25ms)
  cmd.writeUInt16LE(minInterval, 6);

  // Connection Interval Max (units of 1.25ms)
  cmd.writeUInt16LE(maxInterval, 8);

  // Connection Latency (number of connection events, 0 for none)
  cmd.writeUInt16LE(0x0000, 10);

  // Supervision Timeout (units of 10ms, typical default is 2000ms -> 0x00C8)
  cmd.writeUInt16LE(0x00C8, 12);

  cmd.writeUInt16LE(0x0000, 14);
  cmd.writeUInt16LE(0x0000, 16);

  console.log('send connection update - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.processLeMetaEvent = function(eventType, status, data) {
  if (eventType === EVT_LE_CONN_COMPLETE) {
    this.sendConnectionUpdate(
        //0008 is 7.5ms, 000C is 15 and 000F is 20ms
        16,    // Connection handle
        0x000C,    // Min interval (7.5ms)
        0x000F,    // Max interval (15ms)
    );
...
Hci.prototype.processLeConnComplete = function(status, data) {
  ...
  setTimeout(() => {
    this.sendDleUpdate();
    setTimeout(() => {
      this.readMaxDataLength();

      console.log("Read data length request sent.");
    }, 1000);
    /*
    setTimeout(() => {
      ...
      this.sendConnectionUpdate(
          16,    // Connection handle
          0x0006,    // Min interval (7.5ms)
          0x000C,    // Max interval (15ms)
      );
      console.log("Connection update request sent.");
    }, 1000);
*/
    console.log("DLE request sent.");
  }, 500);
