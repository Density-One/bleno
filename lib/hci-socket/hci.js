...
Hci.prototype.setAdvertisingParameters = function() {
  ...
  cmd.writeUInt8(25, 3); // This needs to match the number of parameter bytes.

  // Advertising handle (typically 0 for a single advertiser)
  cmd.writeUInt8(0x00, 4);

  // Advertising properties (e.g., non-connectable, non-scannable, undirected, with extended advertising events)
  cmd.writeUInt16LE(0x0006, 5);

  // Primary advertising interval min (in units of 0.625 ms)
  cmd.writeUInt16LE(advertisementInterval, 7);

  // Primary advertising interval max (in units of 0.625 ms)
  cmd.writeUInt16LE(advertisementInterval, 9);

  // Primary advertising channel map (use all channels)
  cmd.writeUInt8(0x07, 11);

  // Own address type (public)
  cmd.writeUInt8(0x00, 12);

  // Peer address type (public)
  cmd.writeUInt8(0x00, 13);

  // Peer address
  Buffer.from('000000000000', 'hex').copy(cmd, 14);

  // Advertising filter policy (any)
  cmd.writeUInt8(0x00, 20);

  // Advertising TX power (no preference)
  cmd.writeInt8(0x7F, 21);

  // Primary advertising PHY (LE 1M)
  cmd.writeUInt8(0x01, 22);

  // Secondary advertising max skip
  cmd.writeUInt8(0x00, 23);

  // Secondary advertising PHY (LE 2M)
  cmd.writeUInt8(0x02, 24);

  // Advertising SID (use 0 if unused)
   cmd.writeUInt8(0x00, 25);

  // Scan Request Notification Enable (disabled = 0x00, enabled = 0x01)
  cmd.writeUInt8(0x00, 26);
